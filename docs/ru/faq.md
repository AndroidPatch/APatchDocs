...
Недавно появился и активно развивается новый бесплатный форк ZygiskNext с открытым исходным кодом под названием [ReZygisk](https://github.com/PerformanC/ReZygisk). ReZygisk - это новая реализация Zygisk, целью которой является переписать весь код на языке C и обеспечить лучшую реализацию API.

Однако в настоящее время ReZygisk не поддерживает APatch, поэтому мы не рекомендуем использовать ReZygisk сейчас, пожалуйста, подождите, пока он официально поддержит APatch.
